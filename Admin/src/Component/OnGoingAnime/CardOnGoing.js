...
const CardOnGoing = ({index,anime_id,name,image,image_detail,rating,genre,episode}) => {
    ...
    return (
        <div className="col-sm-6 col-md-4 mt-2 mb-2 px-3" style = {{height : '180px'}}  onMouseEnter = {()=>handleEnter(index)} onMouseLeave = {()=>handleLeave(index)}>
            <Link to = {`/detail_on_going/${anime_id}/${name}`} style = {{color : 'black',textDecoration : 'none'}} >
                {/* Hover */}
