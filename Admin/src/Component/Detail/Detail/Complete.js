...
import Size240 from '../Complete/Size240';
import Size360 from '../Complete/Size360';
import Size480 from '../Complete/Size480';
import Size720 from '../Complete/Size720';
import Batch from '../Complete/SizeBatch';
import {withRouter} from 'react-router-dom'
class DetailComplete extends Component {
    ...
    componentDidMount(){
        ...
        if(this._isMounted){
            ...
            this.check_detail(anime_id,name);
    ...
    check_detail = (anime_id,name) => {
        ...
        .then(response => {
            ...
            this.setState({
                ...
                complete_id : response.data.Complete_Anime.complete_id
            })
        })
        .catch(error => {
            this.setState({
    ...
    handleUpdate = () => {
        ...
        .then(response => {
            this.props.history.push(`/detail_complete/${anime_id}/${name}`)
        })
        .catch(error => {
            console.log(error)
        })
    ...
    handleUpToDate = () => {
        ...
        axios({
            method : 'PUT',
            url : `http://localhost:5000/complete/up_to_date_complete/${anime_id}`,
            cancelToken : this.source.token
        })
        .then(response => {
            console.log(response.data)
    ...
    render() {
        ...
        const ViewSize = () => {
            if(this.state.size === "240"){
                return <Size240 complete_id = {this.state.complete_id}/>
            }else if(this.state.size === "360"){
                return <Size360 complete_id = {this.state.complete_id}/>
            }else if(this.state.size === "480"){
                return <Size480 complete_id = {this.state.complete_id}/>
            }else if(this.state.size === "720"){
                return <Size720 complete_id = {this.state.complete_id}/>
            }else if(this.state.size === "Batch"){
                return <Batch complete_id = {this.state.complete_id}/>
            }
        }
                ...
                <div className = "row">
                        ...
                        <div className = "row">
                            ...
                            <div className = "col-6" style = {{textAlign : 'left'}}>
                                ...
                                <div className = "row mt-2">
                                    <button className = "col-5 btn btn-outline-success mx-auto" style = {{width : '100%'}} onClick = {this.handleUpdate}>Update Data</button>
                                    <button className = "col-5 btn btn-success mx-auto" style = {{width : '100%'}} onClick = {this.handleUpToDate}>Up To Date Anime</button>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className = "col-12 mt-5">
                        <div className = "row">
                            ...
                            <div className = "col-2 mx-auto">
                                <button className = "btn btn-primary" style = {{width : '100%'}} onClick = {this.handleBatch}>Batch</button>
                            </div>
                        </div>
                    </div>
                </div>
...
export default withRouter(DetailComplete)