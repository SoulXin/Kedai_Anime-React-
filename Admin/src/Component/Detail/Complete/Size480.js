...
export default class Size480 extends Component {
    ...
    handleUpdate = (store,id,index,complete_id,link) => {
        if(store === "googledrive"){
            ...
            .then(response => {
                ...
                alert(`Data Episode ${index + 1} Untuk GOOGLEDRIVE - 480 Sudah DiPerbarui`)
            })
            .catch(error => {
                console.log(error)
            })
            ...
            .then(response => {
                ...
                alert(`Data Episode ${index + 1} Untuk ZIPPYSHARE - 480 Sudah DiPerbarui`)
            })
            .catch(error => {
                console.log(error)
            })
    ...
    render() {
        ...
        googledrive.map((list,index) => {
            return <tr key = {index}>
                <td>{list.episode}</td>
                <td>
                    <input id = {"googledrive" + index} className = "form-control" type = "text" defaultValue = {list.size480} style = {{width : '100%'}}/>
                </td>
                <td>
                    <button className = "btn btn-primary" onClick = {()=> this.handleUpdate("googledrive",list.googledrive_id,index,list.complete_id,list.size480)} style = {{width : '160px'}}>Update Episode {list.episode}</button>
                </td>
                <td>
        ...
        zippyshare.map((list,index) => {
            return <tr key = {index}>
                <td>{list.episode}</td>
                <td>
                    <input id = {"zippyshare" + index} className = "form-control" type = "text" defaultValue = {list.size480} style = {{width : '100%'}}/>
                </td>
                <td>
                    <button className = "btn btn-primary" onClick = {()=> this.handleUpdate("zippyshare",list.zippyshare_id,index,list.complete_id,list.size480)} style = {{width : '160px'}}>Update Episode {list.episode}</button>
                </td>
                <td>
        ...
        return (
            <div className = "container-fluid mb-5">
                <div className = "row">
                    <h1 className = "col-12 mt-2 mb-1" style = {{textAlign : 'center'}}>LINK UKURAN VIDIO 480</h1>
                    {/* Googledrive */}
                    <div className = "col-6 mt-2" style = {{textAlign : 'center'}}>
                        ...
                        <div className = "row px-2">
                            <div className = "col-12 px-0"  style = {{border : '1px solid black',borderRadius : '5px'}}>
                                ...
                                <button className = "btn btn-primary mt-1 mb-1" onClick = {()=>this.handleShow("googledrive")}>Tambah Link Googledrive versi 480</button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className = "col-6 mt-2" style = {{textAlign : 'center'}}>
                        ...
                        <div className = "row px-2">
                            <div className = "col-12 px-0"  style = {{border : '1px solid black',borderRadius : '5px'}}>
                                ...
                                <button className = "btn btn-primary mt-1 mb-1" onClick = {()=>this.handleShow("zippyshare")}>Tambah Link Zippyshare versi 480</button>
                            </div>
                        </div>
                    </div>
